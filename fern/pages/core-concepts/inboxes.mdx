...
People are used to the traditional Gmail limitations — like having only one inbox. That's a thing of the past.

An `Inbox` is now a fully loaded, programmatically accessible API resource re-designed for the scale of AI Agents.

Think of it as similar to a Gmail or an Outlook account, but built API-first. Each `Inbox` has a unique email address and serves as the primary resource your agent uses to send and receive emails, giving it a first-class identity on the internet.
...
<Tip>
  Psst! Rather than sending 1,000 emails from one `Inbox`, sending 10 emails
...
<Tip>
  ...
  don't provide them, AgentMail generates a unique address for you using our
  default domain. Check out our [guide to managing
