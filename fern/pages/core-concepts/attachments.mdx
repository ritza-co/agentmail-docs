...
If you know the `Message` that an `Attachment` belongs to, you can retrieve it directly.
...
Similarly, you can retrieve an `Attachment` if you know which `Thread` it's in, which can be more convenient for multi-message conversations.
