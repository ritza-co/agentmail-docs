...
Pods are an isolated abstraction that sits between your **organization** and your **inboxes**, providing a method for segmenting and organizing email infrastructure for multi-tenant applications. If you're building a service that offers email functionality to your customers, pods are your key to ensuring customer resource isolation.
...
- Create one pod per customer or tenant in your system
- Pods provide complete isolation between your customers' data
- All resources (inboxes, domains, threads, and drafts) can be scoped to a pod
...
Pods enable you to offer AgentMail's email infrastructure to your own customers while maintaining strict data isolation. Here's how our different customers use pods:

**SaaS and Agency Platforms**: Create a pod for each customer account. Each customer gets their own isolated email workspace.
...
As a basis, here is some logistical stuff that happens on the API side when you create resources.

- When you sign up, the API automatically creates you a `Default Pod`, and all the resources you create (whether they are  `Inboxes` or `Domains`) are associated with this `Default Pod`.
...
<Callout>
  NOTE: As of now, you can only scope domains either to one pod, or to all pods. I.E.,
...
<Tip>
  ...
  uniquely identify pods. That way, you don't need to create a table mapping every
  `organization_id` (for your customers or segments of your business) to its
  equivalent AgentMail `pod_id`. You can just set the `client_id` as your internal id,
...
<Info>
  ...
  and drafts) is automatically cleaned up. You don't need to manually delete
  individual threads or messages.
</Info>

**What's NOT isolated to a pod:**

- Organization-level API keys (these can access any resources in any pod).

Use [pod-scoped API keys](/multi-tenancy#pod-scoped-keys) to restrict access to a single pod, or [inbox-scoped API keys](/multi-tenancy#inbox-scoped-keys) to restrict access to a single inbox.
...
<Accordion title="Can Inboxes Communicate Between Pods?">
  Yes! Inboxes in different pods can send and receive emails to and from one
  another, just like any other email addresses. Pods only provide organizational
