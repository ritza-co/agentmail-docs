...
`Labels` are simple, string-based tags that you can attach to your `Messages` and `Threads`. They are the primary mechanism for organizing, categorizing, and managing the state of your conversations, whether you need to automatically bucket threads into specific categories for your outbound campaign, segment warm leads for your outreach, or categorize inbound into low-ticket, medium-ticket, and high-ticket customers.
...
<Steps>
  ...
  <Step title="Campaign Tracking">
    ...
    `Label`, like `q4-2024-outreach` or `mercor-campaign`, and add a second
    tag such as `warm-lead`. This allows you to easily filter for and analyze the
  ...
  <Step title="Read/Unread Tracking">
    ...
    filter by `labels=["unread"]` to fetch only new messages — preventing your
...
- **Be consistent:** Establish a clear and consistent naming convention for your labels (e.g., `kebab-case` or `snake_case`).
