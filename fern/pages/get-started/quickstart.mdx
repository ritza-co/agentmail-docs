...
For agents that sign themselves up programmatically, no Console or dashboard is needed. The agent registers itself using an email you provide and receives an API key in response. This flow is for first-time users only: human email addresses already signed up with AgentMail will not work here.
...
Copy one of the blocks below into Cursor or Claude for a complete, working AgentMail integration. Each block includes setup, the API reference, error handling, rate limiting, and idempotency guidance.
